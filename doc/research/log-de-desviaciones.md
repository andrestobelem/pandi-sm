...
| DEV-025 | El estado de instancia de excepción (`messageText`, `pendingAction`, `activeHandler`) se almacena en un side-Map JS por-instancia, NO en ivars indexadas, porque `instSize` NO es acumulativo por la cadena de superclases: una subclase de `Exception` queda con `instSize` = sólo sus ivars propias (0 si no declara), y `basicNew` no reservaría el slot heredado de `messageText`. Workaround que sortea esa limitación sin reescribir el modelo. **LIMITACIÓN RAÍZ a cerrar en L4:** `subclass:instanceVariableNames:` debe sumar el `instSize` de la superclase (las colecciones de L4 con ivars heredadas lo necesitan). | — (límite del object model) | ingeniería | §5.5 (L5); raíz a cerrar en §5.4/L4 | decidida |
